...
class MenuAlliance(_Menu):
    ...
    BTN_TERRITORY = Button("TERRITORY", P(1275, 578), P(1385, 660))
    ...
    @classmethod
    def _get_btn_territory(cls):
        if vision.cv.match_region_with_template(
            cls.BTN_TERRITORY, vision.image.RokImages.BTN_TERRITORY
        ):
            return cls.BTN_TERRITORY
    ...
    class MenuAllianceTerritory(_Menu):
        ...
        def open(cls):
            ...
            MenuAlliance.open()
            super().open()
            MenuAlliance.BTN_TERRITORY = MenuAlliance._get_btn_territory()
